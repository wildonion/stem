...
▶ ownership, borrowing, interface and Rust lang concepts:
        ...
        → an eventloop is a jobq loop which is constantly listening to receive async tasks and execute them in a separate light thread 
...
▶ tryout:
        ...
        - distributed serverless actor smart contract functions with global storage allocation: 
                ...
                actor realtime notif handler with cron scheduler(rmq, kafka, redis, rpc, ws): 
                        => Notif actor contract over jobq based channels like mpsc, rmq, Kafka, redis 
                        => actor contract scheduler with tokio time loop, redis pubsub and ctx
                        =>‌ actor tasks execution with async io and cpu tasks threadpool in a blocking 
                           and suspending way with mailbox mpsc channels, mutex and select 
                        ex: atomic minting (channels, mutex, spawn, await, select, notif broker actor and scheduler)
                        receive notifs in its actor through rmq, redis, rpc, ws, kafka (while let some) 
                        store and chache notif data in db and on redis 
                        cron scheudler to call io constantly using tokio time, ctx, redis pubsub exp key
                        send owner notifs to a ws channel for realtime streaming or through short polling http api to update client 
